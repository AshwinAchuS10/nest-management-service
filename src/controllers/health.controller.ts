...
import { HealthCheck, HealthIndicatorResult, MongooseHealthIndicator } from '@nestjs/terminus';
...
export class HealthController {
    ...
    async checkUserService(): Promise<HealthCheckResponse> {
        ...
        try {
            ...
            let databasePingResponse: HealthIndicatorResult = await this.db.pingCheck(DATABASE_NAME).catch(error => error);
            return {
                database: {
                    status: `${DATABASE_NAME}-${databasePingResponse[DATABASE_NAME]?.status == 'up' ? 'IS_ALIVE' : 'IS_DOWN'}`
                },
                service: {
                    status: `MANAGEMENT_SERVICE-IS_ALIVE`
                }
            };
